...
ASR_BACKENDS: Dict[str, BackendSpec] = {
    ...
    # Legacy alias for an unspecified/custom ASR backend.  This alias is
    # accepted in configuration for compatibility with older tests and will
    # resolve to the mock backend during validation.
    "dummy": BackendSpec("dummy", "Dummy ASR", "asr", None, "Alias for the mock ASR backend."),
}
...
def summarize_config_algorithms(config: object) -> Dict[str, str]:
    ...
    return {
        ...
        "asr_model_name": getattr(getattr(config, "asr", None), "model_name", None)
        or getattr(getattr(config, "asr", None), "model", None)
        or "default",
