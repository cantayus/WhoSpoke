...
class AnalysisResult(BaseModel):
    ...
    def to_csv(self, path: str | Path) -> None:
        ...
        The CSV contains one row per final segment with start, end, speaker
        and text.  Additional columns capture per‑modality scores and the
        algorithm trace recorded by the pipeline.  Missing values are
        serialized as empty strings for simplicity.
        ...
        with output_path.open("w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            # write a header including evidence and algorithm trace columns
            writer.writerow([
                "segment_id",
                "start",
                "end",
            ...
            for seg in self.final_segments:
                # convert optional dicts to JSON-like strings or empty
                evidence_str = "" if not seg.evidence_scores else str(seg.evidence_scores)
                alg_trace_str = "" if not seg.algorithm_trace else str(seg.algorithm_trace)
                writer.writerow([
                    seg.segment_id,
                    seg.start,
                    seg.end,
