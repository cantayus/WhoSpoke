...
Most diarization tools answer only **“who spoke when?”** by assigning anonymous labels such as `SPEAKER_00` and `SPEAKER_01`. They do not know whether `SPEAKER_00` is Emmanuel Macron, Donald J. Trump, a journalist, or a moderator.
...
If you do not install these extras up front, WhoSpoke will still attempt to
automatically install the underlying Python packages the first time you
invoke a specific backend (e.g. when selecting the `whisper` ASR or
`pyannote` diarization in your configuration).  Pre‑installing extras
ensures predictable versions and removes the need for runtime downloads, but
...
Imagine a video of a multilingual press conference where two leaders switch between French and English. We want a timestamped transcript with named speakers.
...
<table align="center">
  <tr>
    ...
    <td align="center" width="300">
      <img src="assets/trump.jpg" alt="Donald J. Trump" width="200"><br>
...
This example enrolls only Emmanuel Macron and Donald J. Trump.
...
people:
  ...
  - id: donald_j_trump
    ...
    portraits:
      ...
      - assets/trump_2.jpg
    voice_samples:
      - assets/trump_voice_1.wav
      - assets/trump_voice_2.wav
```
...
```bash
WhoSpoke analyze macron_trump_summit.mp4 \
  --people people.yaml \
...
people = [
    ...
    PersonProfile(
        id="donald_j_trump",
        name="Donald J. Trump",
        portraits=["assets/trump_1.jpg"],
        voice_samples=["assets/trump_voice_1.wav"],
    ),
]

config = AnalysisConfig.from_yaml("analysis.yaml")
analyzer = SpeakerVideoAnalyzer(config=config)
result = analyzer.analyze("macron_trump_summit.mp4", people=people)
...
0:06.5 – 0:12.2  Donald J. Trump: That is right, Emmanuel. We need to work together to address this challenge.
